...
function parseMarketText(text) {
  ...
  const marketPattern =
    /(scorer|score|scores|mål|mal|to score|next team to score|neste lag til å score|neste lag til a score|over\/under|btts|both teams to score|cards?|assists?|målgivende|super sub)/i;

  for (const line of lines) {
    if (!marketPattern.test(line)) {
      continue;
    }

    const cleaned = normalizeWhitespace(
      line
        .replace(/\b(vunnet|tapt|apen|åpent|single|singel|double)\b/gi, '')
        .replace(/\d+[\.,]\d{2}/g, '')
    );
...
function parseOddsenTicket(text, fallbackName) {
  ...
  return {
    ...
    marketText: parseMarketText(text),
    stake,
    odds,
    status: 'pending',
    legs: dedupedLegs,
...
function parseSingleBet(rawText, fallbackName) {
  ...
  const marketText = parseMarketText(rawText);
