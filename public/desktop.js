...
function renderTableRows(bets) {
  ...
  if (!bets.length) {
    desktopList.innerHTML = '<tr><td colspan="16" class="empty">No bets match current filters.</td></tr>';
    return;
  }
    ...
    .map((bet) => {
      ...
      return `
        <tr class="history-row row-${bet.status}">
          ...
          <td>${escapedTypeLabel}</td>
          <td>${gamesCount || '-'}</td>
          <td class="num">${formatCurrency(bet.stake)}</td>
          <td class="num">${Number(bet.odds || 0).toFixed(2)}</td>
